...
try:
    ...
    _WIN_MODAL = Qt.WindowModal  # type: ignore[attr-defined]

try:
    _MB_YES = QMessageBox.StandardButton.Yes
    _MB_NO = QMessageBox.StandardButton.No
except AttributeError:
    _MB_YES = QMessageBox.Yes  # type: ignore[attr-defined]
    _MB_NO = QMessageBox.No    # type: ignore[attr-defined]
...
class IstatConfiniPlugin:
    ...
    def download_and_load_data(self):
        ...
        self.progress_dialog.setWindowModality(_WIN_MODAL)
    ...
    def extract_and_load_all(self):
        ...
        try:
            ...
            QgsMessageLog.logMessage(f"Errore: {str(e)}",
                                   "IstatConfiniPlugin", _MSG_CRITICAL)
        ...
        try:
            ...
            if not os.path.exists(temp_target_path):
                ...
                if similar_folders:
                    ...
                    temp_target_path = os.path.join(extract_temp_dir, target_folder)
                    QgsMessageLog.logMessage(f"Uso cartella alternativa: {target_folder}", "IstatConfiniPlugin", _MSG_INFO)
                else:
            ...
            if keep_files:
                ...
                if not delete_zip:
                    ...
                    QgsMessageLog.logMessage(f"File ZIP conservato: {final_zip_path}", "IstatConfiniPlugin", _MSG_INFO)
                else:
                    ...
                    if os.path.exists(final_zip_path):
                        ...
                        QgsMessageLog.logMessage(f"File ZIP eliminato dalla cartella finale: {final_zip_path}", "IstatConfiniPlugin", _MSG_INFO)

                    # Elimina anche il file ZIP dalla directory temporanea
                    if os.path.exists(zip_path):
                        os.remove(zip_path)
                        QgsMessageLog.logMessage(f"File ZIP eliminato dalla directory temporanea: {zip_path}", "IstatConfiniPlugin", _MSG_INFO)
            else:
            ...
            QgsMessageLog.logMessage(f"Errore estrazione confini: {str(e)}", "IstatConfiniPlugin", _MSG_CRITICAL)
            return None
        ...
        try:
            ...
            if keep_files:
                ...
                if not delete_zip:
                    ...
                    final_zip_path = os.path.join(final_target_path, zip_filename)
                    shutil.copy2(zip_path, final_zip_path)
                    QgsMessageLog.logMessage(f"File ZIP griglia popolazione conservato: {final_zip_path}", "IstatConfiniPlugin", _MSG_INFO)
                else:
                    # Elimina il file ZIP dalla cartella finale se presente
                    zip_filename = os.path.basename(zip_path)
                    final_zip_path = os.path.join(final_target_path, zip_filename)
            ...
            QgsMessageLog.logMessage(f"Errore estrazione griglia popolazione: {str(e)}", "IstatConfiniPlugin", _MSG_CRITICAL)
            return None
